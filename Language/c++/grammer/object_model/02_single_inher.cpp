...
void testDerive(Derive &p) {
    ...
    cout <<"调用了打印虚函数" << endl;
    //若地址正确，则调用了Derive类的print函数
    IsPrint();

    //虚函数表第二个地址是派生析构函数, 通过地址调用析构函数()
    ...
    cout << "根据推测出的地址，输出该地址的值：" << *(int*)((uintptr_t*)(&p) + 1) << endl;
    p.getI();

    cout << "推测nonstatic数据成员DeriveI的地址：" << ((int*)((uintptr_t*)(&p) + 1)+1) << endl;
    cout << "根据推测出的地址，输出该地址的值：" << *((int*)((uintptr_t*)(&p) + 1)+1) << endl;
}
