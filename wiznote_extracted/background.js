// 'use strict'; // removed to avoid strict mode issues in service worker context if needed, but mainly to wrap in IIFE if not already safe or just keep as is.
// V3 service worker adaptation
var Wiz_Context = {};
var wizLocalStorage = {
    removeItem: function(key) { delete this[key]; }
};
// Proxy wizLocalStorage to use chrome.storage.local for persistence if needed, 
// but for now, simple in-memory shim might fail across restarts. 
// Ideally we rewrite wizLocalStorage usage to async chrome.storage.local usage.
// For a quick fix, we'll try to keep it in memory (Wiz_Context resets anyway).
// But wait, the original code uses wizLocalStorage to persist user info. 
// We MUST replace wizLocalStorage with chrome.storage.local.

(function () {
    // Wait for imports if needed? No, importScripts is synchronous.
    
    // Helper to sync load storage (impossible in SW, must be async). 
    // We will have to refactor logic to be async.
    // However, extensive refactoring is risky. 
    // Let's try to initialize critical data from storage first.
    ...
    var _data = {
        ...
        getLocalStorage: function (dataName, timeName, overTime) {
            // Service Worker cannot use localStorage synchronously.
            // This entire logic needs to be async.
            // Returning empty string for now to force fetch from remote,
            // or we need to refactor caller to await.
             return ""; 
             /*
            var localStr = wizLocalStorage[dataName],
                storedTimeStr = wizLocalStorage[timeName],
        ...
        saveUserInfo: function () {
            ...
            chrome.storage.local.set({[Wiz.Constant.LOCAL_STORAGE.USER]: JSON.stringify(userObj)});
        }
    };
    ...
    var handler = {
            ...
            switch (name) {
                ...
                case 'popupClosed':
                    port.onDisconnect.addListener(function () {
                        // V3 Service Worker lifecycle is different; 
                        // disconnect might happen frequently. 
                        // But logic seems ok for when popup explicitly disconnects?
                        // Actually 'popupClosed' is a port name, so this is explicit.
                ...
                case 'pagePreviewFailure':
                     // Handle pagePreviewFailure connect from content script or popup
                     break;
            }
        },
        onGetCategory: function (data, error, port) {
            ...
            if (data && data.code === 200) {
                ...
                wizLocalStorage[CONST.COOKIE.CATEGORY] = locationStr;
                wizLocalStorage[CONST.COOKIE.CATEGORY_TIME] = new Date().valueOf().toString();
                result = locationStr;
            } else {
                wizLocalStorage.removeItem(CONST.COOKIE.CATEGORY);
                wizLocalStorage.removeItem(CONST.COOKIE.CATEGORY_TIME);
                result = {
...
//                console.log('docGuid:' + docGuid + ' title:' + title + ' message' + chrome.i18n.getMessage('clipResult_success'));
        ...
        onGetTag: function (data, error, port) {
            ...
            if (data && data.code === 200) {
                ...
                wizLocalStorage[CONST.COOKIE.TAG] = tagNameList.join(',');
                wizLocalStorage[CONST.COOKIE.TAG_TIME] = new Date().valueOf().toString();
            } else {
                wizLocalStorage.removeItem(CONST.COOKIE.TAG);
                wizLocalStorage.removeItem(CONST.COOKIE.TAG_TIME);
            }
            ...
            if (port) {
                try {
                    port.postMessage({
                        code: data ? data.code : -1,
                        tags: wizLocalStorage[CONST.COOKIE.TAG]
                    });
                } catch (_err) {
                    console.log('_data.login Error: ' + _err);
                }
        ...
        onPreviewCallback: function (option) {
            if (!option) {
                //当前页面无法剪辑
                // chrome.extension.connect is not valid in SW for self-connect usually, 
                // but checking logic: this is callback from content script via sendRequest.
                // In V3, we shouldn't use chrome.extension.connect.
                // Just log or ignore.
        ...
        onPreviewSubmitCallback: function (option) {
            ...
            if (!option && Wiz_Context.tab && Wiz_Context.tab.status === 'complete') {
                var pageClipFailure = chrome.i18n.getMessage('pageClipFailure');
                // alert(pageClipFailure); // Alert not available in SW
                console.warn(pageClipFailure);
            }
        },
    ...
    function getCurTab(callback, params) {
        chrome.tabs.query({active: true, currentWindow: true}, function (tabs) {
            // Check if tabs is valid and has length
    ...
    function wizSavePageContextMenuClick(info, tab) {
        ...
        // In V3 SW, we can't use window.alert() in callbacks like onPreviewSubmitCallback.
        // We must ensure the callback doesn't try to alert.
        Wiz.Browser.sendRequest(tab.id, {
            name: 'preview',
            op: 'submit',
            info: info,
    ...
    function wiz_initContextMenus() {
        ...
        chrome.contextMenus.onClicked.addListener(function(info, tab) {
            if (info.menuItemId === menuId) {
                wizSavePageContextMenuClick(info, tab);
            }
        });
    }
...
// 从api.wiz.cn获取openapi地址
    ...
    chrome.runtime.onConnect.addListener(handler.onConnectListener);
    ...
    setTimeout(function () {
        ...
        chrome.storage.local.get(Wiz.Constant.LOCAL_STORAGE.USER, function(items) {
             if (items && items[Wiz.Constant.LOCAL_STORAGE.USER]) {
                 try {
                     var user = JSON.parse(items[Wiz.Constant.LOCAL_STORAGE.USER]);
                     Wiz_Context.userName = user.name;
                     Wiz_Context.userId = user.id;
                     Wiz_Context.userGuid = user.guid;
                 } catch(e) {}
             }
        });
    }, 300);
