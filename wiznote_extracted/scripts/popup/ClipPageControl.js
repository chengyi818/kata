...
function ClipPageControl() {
    ...
    function initClipPageListener() {
        ...
        // V3: Sync user info from chrome.storage.local to localStorage for compatibility
        chrome.storage.local.get(Wiz.Constant.LOCAL_STORAGE.USER, function(items) {
            if (items && items[Wiz.Constant.LOCAL_STORAGE.USER]) {
                localStorage[Wiz.Constant.LOCAL_STORAGE.USER] = items[Wiz.Constant.LOCAL_STORAGE.USER];
            }
        });
    ...
    //监听截取信息事件
    // V3 update: chrome.extension.onConnect -> chrome.runtime.onConnect
...
//        console.log('requestPageStatus');
    ...
    function requestTag() {
        ...
        port.onMessage.addListener(function(msg) {
            if (msg && msg.tags) {
                localStorage[Wiz.Constant.COOKIE.TAG] = msg.tags;
            }
        });
    }
    ...
    function requestCategoryHandler(msg) {
        ...
        if (typeof msg === 'string'){
            //console.log(msg);
            // V3: Sync to localStorage for ZtreeController
            localStorage[Wiz.Constant.COOKIE.CATEGORY] = msg;
            parseWizCategory();
