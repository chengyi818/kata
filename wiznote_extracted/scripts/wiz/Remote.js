...
(function() {
    ...
    function sendRequest (apiObj, params, callback, callError, options) {
        ...
        var _callError = function (jqXHR, status, error) {
            ...
            if (jqXHR.status !== 413 && apiObj.errCount < CONST.API_COMMON.SERVER_ERROR_RETRY_TIMES) {
                reTryCall();
                return;
            }
    ...
    function fixKsUrl(url) {
        if (curKbServer) {
            url = url.replace(/^.*\/ks\//i, curKbServer + '/ks/');
        }
        return url;
    }
    ...
    Wiz.Remote = {
        ...
        getCategory: function(params, callback) {
            sendRequest({
                url: initApiUrl(fixKsUrl(CONST.API.GET_CATEGORY.url), {
        ...
        getSaveStatus: function(params, callback) {
            sendRequest({
                url: initApiUrl(fixKsUrl(CONST.API.GET_SAVE_STATUS.url), {}),
                action: CONST.API.GET_SAVE_STATUS.action
            }, params, callback);
        },
        getTag: function(params, callback) {
            sendRequest({
                url: initApiUrl(fixKsUrl(CONST.API.GET_TAG.url), {
                    kbGuid: params.kbGuid
                }),
        ...
        saveToServer: function(params, callback) {
            sendRequest({
                url: initApiUrl(fixKsUrl(CONST.API.SAVE_TO_SERVER.url), {}),
                action: CONST.API.SAVE_TO_SERVER.action,
                sendDataType: CONST.API.SAVE_TO_SERVER.sendDataType
            }, params, callback);
        },
        setKbServer: function(kbServer) {
            if (kbServer) {
                curKbServer = kbServer;
            }
        },
        utils: {
