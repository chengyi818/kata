...
- 允许使用AI编程工具
- 交付物：需求澄清文档 + 设计文档 + 代码 + 测试用例（覆盖率≥60%）+ commit记录
...
**交付要求**：需求澄清文档 + 设计文档 + 代码 + 测试用例（覆盖率≥60%）+ commit记录
...
给战斗系统增加多种攻击方式：
- 普通攻击：使用当前的伤害计算逻辑
- 魔法攻击：与魔力值和魔法防御相关，部分穿透物理防御
- 暴击攻击：有一定概率造成额外伤害，附带真实伤害
...
- **Python**：Python 3.10+, unittest（stdlib）, coverage（覆盖率报告）
- **C++**：CMake 3.14+, g++/clang++, Google Test（本地打包）
- **Java**：JDK 11+, Gradle 7+, JUnit 5
- 无需网络下载，所有依赖已预装或本地打包
