						  ...
						  int is_async)
{
    ...
    // 非重点
    // 找不到大于等于buffer_size的binder_buffer,报错退出
	if (best_fit == NULL) {
    ...
    // 为binder_buffer->data分配物理内存
	ret = binder_update_page_range(alloc, 1,
	    (void *)PAGE_ALIGN((uintptr_t)buffer->data), end_page_addr);
	if (ret)
		return ERR_PTR(ret);

    // 新建一个binder_buffer,管理剩余的虚拟内存空间
	if (buffer_size != size) {
		struct binder_buffer *new_buffer;
				   ...
				   struct binder_buffer *buffer)
{
	...
	buffer->free = 1;
    // 合并binder_buffer和下一个binder_buffer
	if (!list_is_last(&buffer->entry, &alloc->buffers)) {
    ...
    // 合并binder_buffer和上一个binder_buffer
