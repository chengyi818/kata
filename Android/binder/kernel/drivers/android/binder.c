...
/**
 * binder_transaction_buffer_release - 清理binder_buffer->data中的特殊资源
 */
...
retry:
    ...
    while (1) {
        ...
        // 从binder_transaction->binder_buffer中,构建binder_transaction_data
        if (put_user(cmd, (uint32_t __user *)ptr)) {
            if (t_from)
    ...
    struct binder_proc *proc, struct binder_thread *new_thread)
{
    ...
    // 在现有binder_proc->threads中,搜索当前thread对应的binder_thread
    while (*p) {
        parent = *p;
    ...
    if (!new_thread)
        return NULL;
    // 将new_thread作为当前linux thread的管理结构体
    thread = new_thread;
