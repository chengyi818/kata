...
struct Task {
    ...
    struct promise_type {
        ...
        std::suspend_never initial_suspend() noexcept {
            std::cout << "    [promise] initial_suspend: 协程开始执行\n";
            return {};
        }
        ...
        std::suspend_always final_suspend() noexcept {
            std::cout << "    [promise] final_suspend: 协程执行完毕\n";
            return {};
        }

        // 4. co_return value; 调用此函数
        void return_value(T value) {
            std::cout << "    [promise] return_value: 存储返回值\n";
...
struct PrintAwaiter {
    ...
    bool await_ready() const noexcept {
        std::cout << "    [awaiter] await_ready: 返回 false，需要挂起\n";
        return false;
    }

    // 挂起时的处理
    void await_suspend(std::coroutine_handle<> handle) const noexcept {
        std::cout << "    [awaiter] await_suspend: 协程已挂起，执行自定义逻辑\n";
        std::cout << "    [awaiter] 消息: " << message << "\n";
    ...
    std::string await_resume() const noexcept {
        std::cout << "    [awaiter] await_resume: 协程恢复，返回结果\n";
        return "awaiter 的结果: " + message;
    }
};
...
Task<std::string> simple_coroutine() {
    ...
    std::cout << "  即将 co_await PrintAwaiter...\n";
    std::string result = co_await PrintAwaiter{"Hello, Coroutine!"};
    std::cout << "  co_await 返回: " << result << "\n";
...
int main() {
    ...
    std::cout << "\n2. 检查协程状态: done=" << std::boolalpha << task.done() << "\n";

    std::cout << "3. 获取结果: " << task.result() << "\n";

    std::cout << "\n=== 执行流程总结 ===\n";
    std::cout << "  1. 调用 simple_coroutine() → 编译器分配协程帧\n";
    std::cout << "  2. 构造 promise_type → 调用 get_return_object() 返回 Task\n";
    std::cout << "  3. initial_suspend() → suspend_never → 立即开始执行函数体\n";
    std::cout << "  4. co_await PrintAwaiter:\n";
    std::cout << "     a. await_ready() → false → 需要挂起\n";
    std::cout << "     b. await_suspend(handle) → 执行自定义逻辑，然后 resume\n";
    std::cout << "     c. await_resume() → 返回结果给 co_await 表达式\n";
    std::cout << "  5. co_return → return_value() → 存储结果到 promise\n";
    std::cout << "  6. final_suspend() → suspend_always → 协程挂起等待销毁\n";
